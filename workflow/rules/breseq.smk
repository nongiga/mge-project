...
rule breseq:
    ...
    output:
        "data/variant_reports/{group}/{sample1}.{sample2}/output/index.html"
    params:
        "data/variant_reports/{group}/{sample1}.{sample2}/"
    threads: config['breseq']['threads']
    priority: config['breseq']['priority']
    log:
        "data/logs/variant_reports/{group}/{sample1}.{sample2}.log"
    conda:
        "../envs/breseq.yaml"
    shell:
